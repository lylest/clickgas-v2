...
const FormAction: FC<IProps> = (
    ...
    return (
        <>
            <FullDivider />
            <div className={twMerge("flex justify-between rounded-lg  bg-gray-100 px-4 py-3 ", className)}>
                <button
                    type="button"
                    onClick={() => navigate(-1)}
                    className={`text-xs ${!onPrevious && "opacity-0 pointer-events-none"} bg-white py-2 px-3 outlined-button`}
                >
                    Previous
                </button>
